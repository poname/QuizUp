...
class Jdf
{
    ...
    function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
        ...
        $T_sec=0;/* <= ÑİÚ ÎØÇí ÒãÇä ÓÑæÑ ¡ ÈÇ ÇÚÏÇÏ '+' æ '-' ÈÑ ÍÓÈ ËÇäíå */
        ...
        for($i=0; $i<$sl; $i++){
            ...
            switch($sub){
                ...
                case'a':
                    $out.=($date[0]<12)?'Ş.Ù':'È.Ù';
                    break;

                case'A':
                    $out.=($date[0]<12)?'ŞÈá ÇÒ ÙåÑ':'ÈÚÏ ÇÒ ÙåÑ';
                    break;
    ...
    function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
        ...
        for($i=0; $i<$sl; $i++){
            ...
            switch($sub){
                ...
                case'p':
                    $out.=($date[1]<12)?'ŞÈá ÇÒ ÙåÑ':'ÈÚÏ ÇÒ ÙåÑ';
                    break;

                case'P':
                    $out.=($date[1]<12)?'Ş.Ù':'È.Ù';
                    break;

                case'r':
                    $out.=$date[0].':'.$date[2].':'.$date[5].' '.(($date[1]<12)?'ŞÈá ÇÒ ÙåÑ':'ÈÚÏ ÇÒ ÙåÑ');
                    break;
        ...
        foreach($array as $type=>$num){
            ...
            switch($type){
                ...
                case'ss':
                    ...
                    if($xy3==1){
                        $p34='';
                        $k34=array('Ïå','?ÇÒÏå','ÏæÇÒÏå','Ó?ÒÏå','åÇÑÏå','ÇäÒÏå','ÔÇäÒÏå','åİÏå','åÌÏå','äæÒÏå');
                        $h34=$k34[substr($num,2-$sl,2)-10];
                    }else{
                        $xy4=substr($num,3-$sl,1);
                        $p34=($xy3==0 or $xy4==0)?'':' æ ';
                        $k3=array('','','È?ÓÊ','Ó?','åá','äÌÇå','ÔÕÊ','åİÊÇÏ','åÔÊÇÏ','äæÏ');
                        $h3=$k3[$xy3];
                        $k4=array('','?˜','Ïæ','Óå','åÇÑ','äÌ','ÔÔ','åİÊ','åÔÊ','äå');
                        $h4=$k4[$xy4];
                    }
                ...
                case'mm':
                    $key=array
                    ('İÑæÑÏ?ä','ÇÑÏ?ÈåÔÊ','ÎÑÏÇÏ','Ê?Ñ','ãÑÏÇÏ','ÔåÑ?æÑ','ãåÑ','ÂÈÇä','ÂĞÑ','Ï?','Èåãä','ÇÓİäÏ');
                    $array[$type]=$key[$num-1];
                    break;

                case'rr':
                    $key=array('?˜','Ïæ','Óå','åÇÑ','äÌ','ÔÔ','åİÊ','åÔÊ','äå','Ïå','?ÇÒÏå','ÏæÇÒÏå','Ó?ÒÏå'
                    ,'åÇÑÏå','ÇäÒÏå','ÔÇäÒÏå','åİÏå','åÌÏå','äæÒÏå','È?ÓÊ','È?ÓÊ æ ?˜','È?ÓÊ æ Ïæ','È?ÓÊ æ Óå'
                    ,'È?ÓÊ æ åÇÑ','È?ÓÊ æ äÌ','È?ÓÊ æ ÔÔ','È?ÓÊ æ åİÊ','È?ÓÊ æ åÔÊ','È?ÓÊ æ äå','Ó?','Ó? æ ?˜');
                    $array[$type]=$key[$num-1];
                    break;
                ...
                case'sh':
                    $key=array('ãÇÑ','ÇÓÈ','æÓİäÏ','ã?ãæä','ãÑÛ','Ó','Îæ˜','ãæÔ','Çæ','áä','ÎÑæÔ','äåä');
                    $array[$type]=$key[$num%12];
                    break;

                case'mb':
                    $key=array('Íãá','ËæÑ','ÌæÒÇ','ÓÑØÇä','ÇÓÏ','ÓäÈáå','ã?ÒÇä','ÚŞÑÈ','ŞæÓ','ÌÏ?','Ïáæ','ÍæÊ');
                    $array[$type]=$key[$num-1];
                    break;
                ...
                case'km':
                    $key=array('İÑ','ÇÑ','ÎÑ','Ê?','ãÑ','Ôå','ãå','ÂÈ','ÂĞ','Ï?','Èå','ÇÓ');
                    $array[$type]=$key[$num-1];
                    break;
    ...
    function gregorian_to_jalali($gy,$gm,$gd,$mod=''){
        $gy=$this->tr_num($gy); $gm=$this->tr_num($gm); $gd=$this->tr_num($gd);/* <= Extra :Çíä ÓØÑ ¡ ÌÒÁ ÊÇÈÚ ÇÕáí äíÓÊ */
    ...
    function gregorian_to_jalali_date($Ymd,$mod=''){
        $exp = explode('-', $Ymd);
        return $this->gregorian_to_jalali(intval($exp[0]), intval($exp[1]), intval($exp[2]), $mod);
    }

    /*	F	*/
    function jalali_to_gregorian($jy,$jm,$jd,$mod=''){
        $jy=$this->tr_num($jy); $jm=$this->tr_num($jm); $jd=$this->tr_num($jd);/* <= Extra :Çíä ÓØÑ ¡ ÌÒÁ ÊÇÈÚ ÇÕáí äíÓÊ */
        $gy=($jy<=979)?621:1600;
        $jy-=($jy<=979)?0:979;
